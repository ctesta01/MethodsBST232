...
and $SSE = \text{Sums of Squares Error}$. 

```{r, echo=FALSE, fig.align='center', out.width='100%'}
knitr::include_graphics("standalone_figures/residual_decompositions/residual_decomposition.svg")
```
...
$$W = \hat \beta_1' \left[ \widehat{\Var}(\hat \beta_1 ) \right]^{-1}  \hat \beta_1$$
...
And thus $$W = (C\hat \beta)'(\widehat{\Var}(C\hat\beta))^{-1}(C\beta)$$
$$ = (C\hat\beta)'[C \widehat{\Var}(\hat\beta) C']^{-1}(C\hat\beta)$$
...
Suppose we want to model the relationship between year in a PhD program and 
some outcome where $X \in \{ 1, 2, 3, 4, 5 \}$ and $Y$ is some continuous outcome,
and we have reason to believe the association might be linear. 

We might model that as $Y \sim \gamma_0 + \gamma_1 X$ (model 1). Another way we could model it that is 
more flexible (not linear) is: 
...
What may not be so obvious is that model 1 is nested inside model 2. 
...
Comparing this to model 2, we see that we need to construct a constraint matrix where 
$$\beta_2 = 2 \beta_1$$
$$\beta_3 = 3 \beta_1$$ 
$$\beta_4 = 4 \beta_1$$
$$\beta_5 = 5 \beta_1$$
